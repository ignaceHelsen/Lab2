...
def train_lin_model(model: LinearRegression, optimizer: torch.optim.Optimizer,
    ...
    for epoch in range(options.num_epochs):
        ...
        for data in train_data:
            ...
            predicted_price = model.forward(size)
            # calculate the loss, use your self created mse loss
            error = mse(predicted_price, price)
            loss += error
        ...
        average_loss = loss / len(train_data.dataset)
        writer.add_scalar("Loss/train", average_loss, epoch)
        test_lin_reg_model(model, train_data, epoch, report=True)

    writer.flush()
    writer.close()


def test_lin_reg_model(model: LinearRegression, test_data: DataLoader, epoch=0, report=False):
    with torch.no_grad():
        loss = 0
        ...
        print(f'Avg error/example: € {average_loss :.2f}\n')
        if report:
...
def test_lin_reg_plot(model: LinearRegression, test_data: DataLoader, options: LinearRegressionOptions):
    ...
    with torch.no_grad():
        for data in test_data:
            plt.scatter(data[:, 0].to('cpu'), data[:, 1].to('cpu'), c="g")
            size, price = data[:, 0].unsqueeze(1), data[:, 1].unsqueeze(1)
            estimated_price = model(size)
            plt.scatter(data[:, 0].to('cpu'), estimated_price.to('cpu'), c="r")
...
def test_classification_model(model: Classifier, dataset: MNISTDataset, options: ClassificationOptions):
    with torch.no_grad():
        ...
        for x, y in dataset.test_loader:
            output = model(x.to(options.device))
            y = y.to(options.device)
